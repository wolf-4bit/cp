...
void solve()
{
    string s;
    cin >> s;
    bool allUppercase = true, isFirstCap = isupper(s[0]);

    for(int i = 1; i < s.length(); i++) if (!isupper(s[i])) allUppercase = false;
    if((allUppercase && isFirstCap) || (!isFirstCap && allUppercase)) process_string(s);
    cout << s;
}
