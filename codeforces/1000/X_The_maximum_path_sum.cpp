...
ll solve(vector<vector<int>>& a, int r, int m)
{
    if (r == 0 && m == 0)
        return a[r][m];
    ll upSum = (r > 0) ? solve(a, r - 1, m) : 0;
    ll rightSum = (m > 0) ? solve(a, r, m - 1) : 0;

    return a[r][m] + max(rightSum, upSum);
}
...
int32_t main()
{
    ...
    int r, m;
    cin >> r >> m;
    vector<vector<int>> a(r,vector<int>(m, 0));
    fr(i,0,r){
        fr(j,0,m){
            cin >> a[i][j];
        }
    }
    cout << solve(a, r - 1, m - 1);
    return 0;
}