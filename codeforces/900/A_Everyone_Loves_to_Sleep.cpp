...
void solve()
{
    int n, h,m;
    cin>>n>>h>>m;
    int caluculatedMins = (h * 60) + m;
    int mincaluculatedMins = INT_MAX;
    for(int i = 0; i < n; i++){
        int tempH, tempM, actualTime;
        cin>>tempH>>tempM;
        int tempCalculatedMins = (tempH * 60) + tempM;
        int remTime = tempCalculatedMins - caluculatedMins;
        if(remTime < 0){
            remTime = remTime + 1440;
        }
        mincaluculatedMins = min(remTime, mincaluculatedMins);
    }

    cout << mincaluculatedMins / 60 << " " << mincaluculatedMins % 60 << endl;
}
